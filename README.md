...
The python project is a simple command-line application that generates current weather conditions and 24-hour forecast for user provided city. 
...
Now the project can be runed via docker: -it -v ${PWD}:/app lvas55/weather-tool:latest

Issues and notes:
Two main issue were encountered in the process of making Docker image and running teh container:
...
- Since the file request user input, teh container does not run smoothly in docker.desktop app (does not wait for input and stops running immediately).
The solution where is to run it via the terminal. Works as expected then.
